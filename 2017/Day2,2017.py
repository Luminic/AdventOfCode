myinput=[[493,	458,	321,	120,	49,	432,	433,	92,	54,	452,	41,	461,	388,	409,	263,	58],
...
for i in range (0, len(myinput)):
    highestnum = 0
    lowestnum = 10000
    for j in range (0, len(myinput[i])):
        if (myinput[i][j] > highestnum):
            highestnum = myinput[i][j]
        if (myinput[i][j] < lowestnum):
            lowestnum = myinput[i][j]
    summation += highestnum-lowestnum
print(summation)
