...
for i in range(0,len(convertedinput)):
    if (convertedinput[i].isalpha()):
        ...
        if (len(currentword) > 0 and not currentword in variables and currentword != "if"):
            variables.append(currentword)
        currentword = ""
...
for i in range(0,len(convertedinput)):
    if (convertedinput[i] != "\n"):
        currentcommand += convertedinput[i]
    else:
...
wordedcommands = []
for com in commands:
    modifiedcommand = list(com)
    words = []
    currentword = ""
    for i in range(0,len(modifiedcommand)):
        if (modifiedcommand[i] != " "):
            currentword += modifiedcommand[i]
        else:
            if (len(currentword) > 0):
                if (currentword == "inc"):
                    currentword = "+="
                elif (currentword == "dec"):
                    currentword = "-="
                words.append(currentword)
            currentword = ""
    wordedcommands.append(words)

toexecute = []
for finalcommand in wordedcommands:
    thefirsthalfcommand = ""
    thesecondhalfcommand = ""
    for final in finalcommand:
        if (final == "if"):
            thesecondhalfcommand = thefirsthalfcommand
            thefirsthalfcommand = "if "
        else:
            thefirsthalfcommand += final
    toexecute.append(thefirsthalfcommand + ": " + thesecondhalfcommand)

del toexecute[0]
for execute in toexecute:
    #print(execute)
    exec(execute)
...
for var in variables:
    exec("print("+var+",var)")
    exec("if ("+var+">maximum): maximum = "+var)
print(maximum)
